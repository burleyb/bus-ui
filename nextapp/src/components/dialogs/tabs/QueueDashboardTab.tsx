...
export default function QueueDashboardTab({ nodeData, timePeriod, onClose }: QueueDashboardTabProps) {
  ...
  const { 
    ...
    error: detailsError 
  } = useNodeDetailsData(nodeData?.id, 'queue', timePeriod, 60000);
  ...
  const findTimeSeriesData = (data: any, possibleKeys: string[]): any[] => {
    ...
    for (const key of possibleKeys) {
      if (data[key] && Array.isArray(data[key]) && data[key].length > 0) {
        console.log(`[QueueDashboardTab] Found data in top-level property '${key}'`);
        return data[key];
      }
    }
      ...
      if (data[nestedKey] && typeof data[nestedKey] === 'object') {
        ...
        for (const key of possibleKeys) {
          if (nestedData[key] && Array.isArray(nestedData[key]) && nestedData[key].length > 0) {
            console.log(`[QueueDashboardTab] Found data in nested property '${nestedKey}.${key}'`);
    ...
    if (possibleKeys.includes('eventsWritten') && data.executions && Array.isArray(data.executions)) {
      console.log(`[QueueDashboardTab] Using 'executions' data for events written`);
      return data.executions;
    }
    
    if (possibleKeys.includes('lag') && data.duration && Array.isArray(data.duration)) {
      console.log(`[QueueDashboardTab] Using 'duration' data for lag`);
            ...
            for (const botId in data[section][direction]) {
              ...
              if (bot.values && Array.isArray(bot.values) && bot.values.length > 0) {
                if ((possibleKeys.includes('eventsWritten') && direction.includes('write')) || 
                  ...
                  console.log(`[QueueDashboardTab] Found data in bot '${botId}' in direction '${direction}'`);
                  return bot.values;
                }
              }
            }
    ...
    console.log(`[QueueDashboardTab] Could not find data for keys: ${possibleKeys.join(', ')}`);
    return [];
  };
  ...
  const prepareBotData = () => {
    ...
    // Handle metrics for bots that write to this queue
    
    // Handle metrics for bots that read from this queue
    ...
    <div className="flex flex-col h-full" style={{ minHeight: "calc(100vh - 240px)" }}>
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-grow">
          {/* Events written to and read from queue - side by side in md screens and larger */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            {/* Events written by bots to this queue */}
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
              <h3 className="text-lg font-medium mb-4">Events Written By Bots To This Queue</h3>
              ...
              {nodeData && nodeData.bots?.write && Object.keys(nodeData.bots.write).length > 0 ? (
                ...
                <div className="text-center py-8 text-gray-500 dark:text-gray-400">No bots writing to this queue</div>
              )}
            ...
            {/* Events read by bots from this queue */}
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
              <h3 className="text-lg font-medium mb-4">Events Read By Bots From This Queue</h3>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mt-auto">
          <h3 className="text-lg font-medium mb-4">Queue Performance</h3>
