...
import Dialog from './dialog.jsx'; // Assuming you have a Dialog component
...
function Topics({ onClose }) {
    ...
    const mutation = useMutation(saveTopic, {
        ...
        onError: () => {
            // Handle error
            state.messageLogNotify('Failed to update topic', 'error');
        },
    });

    const saveTopic = async (topic) => {
        const { data } = await axios.post(`${state.api}/api/sns_save/topic/${topic.arn}`, topic);
        return data;
    };

    const fetchSNSData = async () => {
        const { data } = await axios.get(`${state.api}/api/sns_get`);
        return data;
    };
