...
function ListView({ searches, userSettings }) {
    ...
    const dataStore = state; // Assuming dataStore is part of the context state
    ...
    useEffect(() => {
        ...
        const adjustScrollFiller = () => {
            ...
            if (fillerTop) {
                fillerTop.style.height = `${rowHeight * startRow}px`;
            }

            const fillerBottom = element.querySelector('.filler-bottom');
            if (fillerBottom) {
                fillerBottom.style.height = `${rowHeight * (tableData.length - startRow - visibleRowCount)}px`;
            }
        };
            ...
            <div className="top-controls button-group">
                <NodeSearch
                    ...
                    searchResults={(results, text) => {
                        ...
                        if(searches?.current && searches?.current?.text !== text) {
