...
const Checksum = ({ nodeData }) => {
    ...
    const fetchChecksums = async () => {
        try {
            const checksumsData = await axios.get(`${state.api}/api/checksums`, {
                params: {
    ...
    const runNow = async (botId) => {
        try {
            await axios.post(`${state.api}/api/cron/save`, {
                id: botId,
                executeNow: true,
            });
            state.messageLogNotify('Checksum run triggered on ' + state.nodes[botId].label);
        } catch (error) {
            state.messageLogModal('Failure triggering checksum run on ' + state.nodes[botId].label, 'error', error);
        }
    };
    ...
    const restartNow = async (botId) => {
        try {
            ...
            state.messageLogNotify('Checksum restart triggered on ' + state.nodes[botId].label);
        } catch (error) {
            state.messageLogModal('Failure triggering checksum restart on ' + state.nodes[botId].label, 'error', error);
        }
    };
