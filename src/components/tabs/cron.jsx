...
const Cron = ({ nodeData }) => {
	...
	const addCronMutation = useMutation(
		async (newCron) => {
			return axios.post(`${state.api}/system/${nodeData.id}`, newCron);
		},
		{
			onSuccess: () => {
	...
	const addCron = () => {
		...
		// Simulate state.createBot or implement as needed
		state.createBot(newCron);
	};
	...
	const runNowMutation = useMutation(
		async (cronId) => {
			return axios.post(`${state.api}/system/${cronId}`, { executeNow: true });
		},
		{
			onSuccess: (cronId) => {
				state.messageLogNotify(`Cron run triggered on ${nodes[cronId]?.label}`);
			},
			onError: (error, cronId) => {
				state.messageLogModal(
