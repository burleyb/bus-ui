...
const QueueSettings = ({ nodeData, setDirtyState }) => {
  ...
  const handleSave = async (callback) => {
    ...
    try {
      ...
      setDirtyState(false);
      callback && callback();
      state.messageLogNotify(`Queue settings saved successfully for ${nodes[nodeData.id]?.label}`);
    } catch (error) {
      console.error('Error saving queue settings:', error);
      state.messageLogModal(`Failure saving queue ${nodes[nodeData.id]?.label}`, 'error', error);
    }
  };
