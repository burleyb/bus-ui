...
function BotSettings({ action, data, nodeData, onClose, onSave }) {
  ...
  const mutation = useMutation(saveBotSettings, {
    onSuccess: (response) => {
      state.messageLogNotify(`Bot settings saved successfully for "${settings.name}"`);
    ...
    onError: (error) => {
      state.messageLogModal(`Failure saving bot settings for "${settings.name}"`, 'error', error);
    },
  });
  ...
  const toggleArchived = () => {
    ...
    axios.post(`${state.api}/api/cron/save`, { id: nodeData.id, archived: archiveStatus }).then(() => {
      setArchived(archiveStatus);
      state.messageLogNotify(`Bot ${archiveStatus ? 'Archived' : 'Unarchived'}`);
      state.fetchData();
    });
  };

  const fetchBotSettings = async (id) => {
    const { data } = await axios.get(`${state.api}/api/cron/${encodeURIComponent(id)}`);
    return data;
  };

  const saveBotSettings = async (data) => {
    const { id, ...payload } = data;
    const response = await axios.post(`${state.api}/api/cron/save`, JSON.stringify(payload));
